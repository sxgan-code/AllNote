...
${emp.name!"xxx"}    　　　　　　  // 变量不存在，取默认值xxx1234
```
...
$ {“hello”+ emp.name！} //使用+号来连接，输出你好+变量名12
```
...
$ {str [1..4]} //结果是bcde
//返回指定字符的索引
$ {str?index_of（"n"）}12345678
```

#### 2，日期输出

```
$ {emp.date?string（'yyyy -MM-dd'）} //日期格式1
```
...
＃{Y; m2} //输出4.0
＃ {x; m1M2} //输出2.58 
＃ {x; m1M2} //输出4.0 12345678910111213141516171819202122232425
```
...
$ {foo？string（“yes”，“no”）} //当为真时输出“yes”，否则输出“no”   12
```
...
逻辑或：|| 
逻辑非：！
逻辑运算符只能作用于布尔值，否则将产生错误12345
```

#### 8，[FreeMarker](https://so.csdn.net/so/search?q=FreeMarker&spm=1001.2101.3001.7020)中的运算符优先级如下（由高到低排列）：
...
⑦，逻辑与：&& 
⑧，逻辑或：|| 
⑨，数字范围：.. 实际上，我们在开发过程中应该使用括号来严格区分，这样的可读性好，出错少123456789
```
...
// 正确写法
<#if isBig>Wow!</#if> 1234567891011121314151617181920
```
...
　　<a href="${movie.url}">${movie.name}</a>
</#list>12345678910111213141516171819202122232425262728293031323334353637383940414243444546474849505152535455565758596061626364656667686970717273747576
```
...
emp.name       // 全部使用点语法
emp["name"]    // 使用方括号
循环//遍历集合：<＃list empList！as emp>     $ {emp.name！}12345678910
```
...
${r"${foo}"} // 输出 ${foo}
${r"C:/foo/bar"} // 输出 C:/foo/bar1234567891011121314151617
```
...
注意:在ftl文件里布尔值都是直接赋值的如parse=true,而不是parse="true"1234567
```
...
// 上面的代码将导入/lib/common.ftl模板文件中的所有变量,交将这些变量放置在一个名为com的Map对象中，"my"在freemarker里被称作namespace123
```
...
Last name: ${lastName} 
Maiden name: ${maidenName?html}123456789101112
```
...
// 利用这个语法给变量赋值，那么这个变量在所有的namespace中是可见的，
    ...
    在当前页面里x=2将被隐藏，或者通过${.globals.x} 来访问1234567891011
```
...
${1.2} // 输出1.2,因为匈牙利是采用", "作为十进制的分隔符，美国是用". "123456789101112131415
```
...
<@test foo="a"/>                   // 输出：Text: a, Bar, -112345678910
```
...
// 输出Animals *mouse *elephant *python1234567891011
```
...
http://blog.sina.com.cn/s/blog_7e5699790100z59g.html  123456789101112131415161718
```
...
<#-- 使用宏 -->
<@book />
// 上面的代码输出:spring,而j2ee位于return指令之后,不会输出.1234567891011
FreeMarker 工具类：
