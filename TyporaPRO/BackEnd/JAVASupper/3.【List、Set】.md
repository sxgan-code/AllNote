...
`java.util.Set`接口和`java.util.List`接口一样，同样继承自`Collection`接口，它与`Collection`接口中的方法基本一致，并没有对`Collection`接口进行功能上的扩充，只是比`Collection`接口更加严格了。与`List`接口不同的是，`Set`接口中==元素无序== ，并且都会以某种规则保证存入的元素不出现重复。
...
在HashSet下面有一个子类`java.util.LinkedHashSet`，它是链表和哈希表组合的一个数据存储结构，存储数据是有序的。
...
* `java.utils.Collections`是==集合工具类==  ，用来对集合进行操作。部分方法如下：
