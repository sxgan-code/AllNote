...
[安装教程](../../DevEnvironment/Windows/windows10.md)
...
字符串类型指`CHAR、VARCHAR、BINARY、VARBINARY、BLOB、TEXT、ENUM和SET`。该节描述了这些类型如何工作以及如何在查询中使用这些类型。
...
```sql
DROP TABLE IF EXISTS admin;# IF EXISTS存在即删
```
...
- 如果查询字段中包含通配符，则需要使用反斜线 `\` 进行转义操作，例如：`'abc\_%'` 表示查询``'abc_'``开头的所有字段
